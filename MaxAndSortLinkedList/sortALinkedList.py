...
def sort_linked_list(linked_list):
  ...
  #while there is a head_node loop will run
  while linked_list.get_head_node():
    #call the max function to find the max value of a linkedlist
    list_max = find_max(linked_list)
    #we remove the node from the list it is still saved in the list_max
    linked_list.remove_node(list_max)
    #insert the value into our new linked list and it is saved there
    new_linked_list.insert_beginning(list_max)
